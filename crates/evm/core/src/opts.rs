...
use foundry_evm_hardforks::{FoundryHardfork, parse_local_arc_hardfork};
...
impl EvmOpts {
    /// Folds an Arc hardfork selected through `hardfork = "arc:zeroN"` into [`Self::networks`].
    ///
    /// `networks.arc_hardfork` is the only channel the Arc EVM factory reads when picking a
    /// chainspec (`arc_chainspec_for`). `Config::hardfork` alone is not enough, because it only
    /// feeds the revm `SpecId` and every Arc hardfork from Zero5 on maps to `SpecId::OSAKA`.
    /// Without this the selection is silently dropped and execution falls back to the latest
    /// local schedule — anvil already does the same fold in `NodeConfig::with_hardfork`.
    pub const fn apply_arc_hardfork(&mut self, hardfork: Option<FoundryHardfork>) {
        if let Some(FoundryHardfork::Arc(arc_hardfork)) = hardfork
            && self.networks.is_arc()
        {
            self.networks = self.networks.with_arc_hardfork(arc_hardfork);
        }
    }
    ...
    pub async fn infer_network_from_fork(&mut self) {
        ...
        {
            // If Anvil's chain, request anvil_nodeInfo to determine its non-Ethereum network.
            if chain_id == NamedChain::AnvilHardhat as u64 {
                if let Ok(node_info) =
                    provider.raw_request::<_, NodeInfo>("anvil_nodeInfo".into(), ()).await
                {
                    self.networks = match node_info.network.as_deref() {
                        // Inherit the hardfork too, not just the network. Chain id 31337 has no
                        // published hardfork schedule to derive it from, so what the node reports
                        // is the only way to tell a localdev-schedule Anvil apart from a default
                        // one — and simulating the wrong Arc fork changes gas and drops the
                        // Zero7+ subcall precompiles. An unparsable value keeps the default.
                        Some("arc") => match parse_local_arc_hardfork(&node_info.hard_fork) {
                            Ok(hardfork) => NetworkConfigs::with_arc().with_arc_hardfork(hardfork),
                            Err(_) => NetworkConfigs::with_arc(),
                        },
                        Some("tempo") => NetworkConfigs::with_tempo(),
                        _ => self.networks,
                    };
                }
            } else {
...
mod tests {
    ...
    use super::*;
    use foundry_evm_hardforks::ArcHardfork;

    /// `hardfork` and `networks` live on different structs (`Config` vs `EvmOpts`), which is why
    /// an Arc hardfork used to be parsed and then dropped. Pin down that both halves still arrive
    /// from a foundry.toml-shaped figment.
    #[test]
    fn arc_hardfork_and_network_both_arrive_from_config() {
        let figment = Config::figment().merge(("network", "arc")).merge(("hardfork", "arc:zero6"));

        let evm_opts = figment.extract::<EvmOpts>().unwrap();
        let config = Config::from_provider(figment).unwrap().sanitized();

        assert!(evm_opts.networks.is_arc());
        assert_eq!(config.hardfork, Some(FoundryHardfork::Arc(ArcHardfork::Zero6)));
        // Not folded yet: this is exactly the gap `apply_arc_hardfork` closes.
        assert_eq!(evm_opts.networks.arc_hardfork(), None);
    }

    #[test]
    fn apply_arc_hardfork_folds_explicit_selection_into_networks() {
        let mut evm_opts = EvmOpts { networks: NetworkConfigs::with_arc(), ..Default::default() };
        evm_opts.apply_arc_hardfork(Some(FoundryHardfork::Arc(ArcHardfork::Zero6)));
        assert_eq!(evm_opts.networks.arc_hardfork(), Some(ArcHardfork::Zero6));
        assert!(evm_opts.networks.is_arc());
    }

    #[test]
    fn apply_arc_hardfork_is_a_noop_outside_arc_mode() {
    ...
    #[test]
    fn apply_arc_hardfork_ignores_non_arc_hardforks() {
        let mut evm_opts = EvmOpts { networks: NetworkConfigs::with_arc(), ..Default::default() };
        let before = evm_opts.networks.arc_hardfork();
        evm_opts.apply_arc_hardfork(Some(FoundryHardfork::Ethereum(
            foundry_evm_hardforks::EthereumHardfork::Prague,
        )));
        assert_eq!(evm_opts.networks.arc_hardfork(), before);
    }
    ...
    /// Forking an Anvil that opted into a newer Arc fork has to carry that fork across. Chain id
    /// 31337 has no published schedule to derive it from, so `anvil_nodeInfo` is the only source —
    /// without this the simulation silently runs Foundry's default fork against a node that isn't
    /// on it, which changes gas and drops the Zero7+ subcall precompiles.
    #[tokio::test(flavor = "multi_thread")]
    async fn infer_network_arc_anvil_inherits_the_reported_hardfork() {
        // Deliberately not the default, so inheriting and defaulting give different answers.
        let default = foundry_evm_hardforks::default_local_arc_hardfork();
        let selected = *foundry_evm_hardforks::LOCAL_ARC_HARDFORKS
            .iter()
            .find(|hardfork| **hardfork != default)
            .expect("a locally selectable Arc fork other than the default");

        let (_api, handle) = anvil::spawn(
            anvil::NodeConfig::test()
                .with_networks(NetworkConfigs::with_arc().with_arc_hardfork(selected))
                .with_hardfork(Some(selected.into())),
        )
        .await;
        ...
        assert!(evm_opts.networks.is_arc(), "should detect arc via anvil_nodeInfo");
        assert_eq!(
            evm_opts.networks.arc_hardfork(),
            Some(selected),
            "should inherit the forked node's hardfork rather than fall back to {default:?}"
        );
    }
