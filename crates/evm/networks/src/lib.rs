...
use clap::Parser;
use foundry_evm_hardforks::{FoundryHardfork, default_local_arc_hardfork};
...
pub const fn is_arc_chain_id(chain_id: ChainId) -> bool {
    use arc_execution_config::chain_ids::{DEVNET_CHAIN_ID, MAINNET_CHAIN_ID, TESTNET_CHAIN_ID};

    matches!(chain_id, MAINNET_CHAIN_ID | DEVNET_CHAIN_ID | TESTNET_CHAIN_ID)
}
...
pub struct NetworkConfigs {
    ...
    #[arg(help_heading = "Networks", long, short, num_args = 1, value_name = "NETWORK", value_enum, conflicts_with_all = ["celo", "tempo", "arc"])]
    #[cfg_attr(feature = "optimism", arg(conflicts_with = "optimism"))]
    #[serde(default)]
    pub(crate) network: Option<NetworkVariant>,
    /// Enable Celo network features.
    #[arg(help_heading = "Networks", long, conflicts_with_all = ["network", "tempo", "arc"])]
    #[cfg_attr(feature = "optimism", arg(conflicts_with = "optimism"))]
    celo: bool,
    /// Enable Optimism network features (deprecated: use --network optimism).
    #[cfg(feature = "optimism")]
    #[arg(long, hide = true, conflicts_with_all = ["network", "celo", "tempo", "arc"])]
    ...
    #[arg(long, hide = true, conflicts_with_all = ["network", "celo", "arc"])]
    #[cfg_attr(feature = "optimism", arg(conflicts_with = "optimism"))]
    // Deserialize-only legacy alias: accepted in foundry.toml but never serialized — the
    // canonical form is `network = "tempo"`.
    #[serde(default)]
    tempo: bool,
    /// Enable Arc execution semantics (deprecated: use --network arc).
    #[arg(long, hide = true, conflicts_with_all = ["network", "celo", "tempo"])]
    #[cfg_attr(feature = "optimism", arg(conflicts_with = "optimism"))]
    #[serde(default)]
    arc: bool,
    /// Exact Arc hardfork for local execution. `None` uses the selected chain's schedule.
    #[arg(skip)]
    #[serde(default)]
    arc_hardfork: Option<ArcHardfork>,
...
impl Serialize for NetworkConfigs {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        ...
        let mut s = serializer
            .serialize_struct("NetworkConfigs", 3 + usize::from(self.arc_hardfork.is_some()))?;
        ...
        if let Some(hardfork) = self.arc_hardfork {
            s.serialize_field("arc_hardfork", &hardfork)?;
        }
        s.end()
    }
}
...
impl NetworkConfigs {
    ...
    pub fn with_arc() -> Self {
        Self {
            network: Some(NetworkVariant::Arc),
            arc: true,
            arc_hardfork: Some(default_local_arc_hardfork()),
            ..Default::default()
        }
    }

    fn with_detected_arc() -> Self {
        Self { network: Some(NetworkVariant::Arc), arc: true, ..Default::default() }
    }

    pub const fn with_arc_hardfork(mut self, hardfork: ArcHardfork) -> Self {
        self.network = Some(NetworkVariant::Arc);
        self.arc = true;
        self.arc_hardfork = Some(hardfork);
        self
    }

    pub const fn arc_hardfork(&self) -> Option<ArcHardfork> {
        self.arc_hardfork
    }

    pub const fn is_arc(&self) -> bool {
        matches!(self.resolved_network(), Some(NetworkVariant::Arc))
    }
    ...
    const fn resolved_network(&self) -> Option<NetworkVariant> {
        ...
        if self.arc {
            return Some(NetworkVariant::Arc);
        }
        #[cfg(feature = "optimism")]
        if self.optimism {
    ...
    pub fn normalize_for_hardfork(self, hardfork: FoundryHardfork) -> Result<Self, String> {
        ...
        let network = match hardfork {
            ...
            FoundryHardfork::Arc(hardfork) => Self::with_arc().with_arc_hardfork(hardfork),
            #[cfg(feature = "optimism")]
...
impl From<NetworkVariant> for NetworkConfigs {
    fn from(network: NetworkVariant) -> Self {
        match network {
            ...
            NetworkVariant::Arc => Self::with_arc(),
            #[cfg(feature = "optimism")]
...
mod tests {
    ...
    #[test]
    fn arc_flag_and_variant_agree() {
        let via_new = NetworkConfigs { network: Some(NetworkVariant::Arc), ..Default::default() };
        let via_old = NetworkConfigs { arc: true, ..Default::default() };
        assert_eq!(via_new.is_arc(), via_old.is_arc());
        assert_eq!(NetworkConfigs::with_arc().active_network_name(), Some("arc"));
    }

    #[test]
    fn from_str_parses_all_variants() {
        // Exercises the `FromStr` impl used by inline forge-config `network = "..."` parsing
        // (crates/config/src/inline). `"arc"` must round-trip, matching `name()`.
        assert_eq!("ethereum".parse::<NetworkVariant>(), Ok(NetworkVariant::Ethereum));
        assert_eq!("tempo".parse::<NetworkVariant>(), Ok(NetworkVariant::Tempo));
        assert_eq!("arc".parse::<NetworkVariant>(), Ok(NetworkVariant::Arc));
        assert_eq!(NetworkVariant::Arc.name().parse::<NetworkVariant>(), Ok(NetworkVariant::Arc));
        assert!("nope".parse::<NetworkVariant>().is_err());
    }

    #[test]
    fn serde_roundtrip_arc() {
        let original = NetworkConfigs::with_arc();
        let json = serde_json::to_string(&original).unwrap();
        let restored: NetworkConfigs = serde_json::from_str(&json).unwrap();
        assert!(restored.is_arc());
        assert!(!restored.is_tempo());
    }

    #[test]
    fn arc_chain_ids_select_arc_network() {
        use arc_execution_config::chain_ids::{
            DEVNET_CHAIN_ID, MAINNET_CHAIN_ID, TESTNET_CHAIN_ID,
        };

        for chain_id in [MAINNET_CHAIN_ID, DEVNET_CHAIN_ID, TESTNET_CHAIN_ID] {
            assert_eq!(NetworkVariant::from(chain_id), NetworkVariant::Arc);
        }
    }

    #[test]
    fn arc_hardfork_override_is_preserved_separately_from_chain_detection() {
        let explicit = NetworkConfigs::with_arc()
            .normalize_for_hardfork(FoundryHardfork::Arc(ArcHardfork::Zero6))
            .unwrap();
        assert_eq!(explicit.arc_hardfork(), Some(ArcHardfork::Zero6));

        let detected = NetworkConfigs::default()
            .with_chain_id(arc_execution_config::chain_ids::TESTNET_CHAIN_ID);
        assert!(detected.is_arc());
        assert_eq!(detected.arc_hardfork(), None);
    }
