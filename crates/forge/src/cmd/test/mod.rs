...
use foundry_evm::{
    core::evm::{
        ArcEvmNetwork, BlockEnvFor, EthEvmNetwork, FoundryEvmNetwork, SpecFor, TempoEvmNetwork,
        TxEnvFor,
    },
    opts::EvmOpts,
...
impl TestArgs {
    ...
    async fn dispatch_network(
        ...
        if dispatch_opts.networks.is_tempo() {
            ...
            .await
        } else if dispatch_opts.networks.is_arc() {
            self.build_and_run_tests::<ArcEvmNetwork>(
                config,
                evm_opts,
                output,
                filter,
