...
impl GenesisConfig {
    ...
    /// Applies an external [`Genesis`]'s alloc (accounts + storage) to the db.
    ///
    /// Used to deploy the Arc system contracts (ProtocolConfig / SystemAccounting / native system
    /// contracts) for a non-fork `--arc` node, from the Arc chain spec's genesis.
