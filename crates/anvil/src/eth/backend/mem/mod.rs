...
use arc_execution_config::{
    ...
    gas_fee::{decode_base_fee_from_bytes, encode_base_fee_to_bytes},
    native_coin_control::{compute_is_blocklisted_storage_slot, is_blocklisted_status},
};
use arc_precompiles::{NATIVE_COIN_CONTROL_ADDRESS, system_accounting::retrieve_gas_values};
use chrono::Datelike;
...
use foundry_evm::{
    ...
    core::{evm::FoundryArcEvmFactory, precompiles::EC_RECOVER},
...
impl<N: Network> Backend<N> {
    ...
    /// Returns true if Arc network mode is active.
    pub const fn is_arc(&self) -> bool {
        self.networks.is_arc()
    }
        ...
        WrapDatabaseRef<&'db DB>: Database<Error = DatabaseError>,
    {
        ...
        // `op_deposit` only matters on the OP path; arc/eth/tempo ignore it.
        let _ = op_deposit;
        if self.is_arc() {
            return self.transact_arc_with_inspector_ref(db, evm_env, inspector, tx_env);
        }
        if self.is_tempo() {
    ...
    /// Arc path of [`Backend::transact_with_inspector_ref`].
    ///
    /// Arc executes an Ethereum-shaped transaction, but the spec and precompiles come from the
    /// Arc chainspec rather than from `evm_env`.
    fn transact_arc_with_inspector_ref<'db, I, DB>(
        &self,
        db: &'db DB,
        evm_env: &EvmEnv,
        inspector: &mut I,
    ...
    where
        ...
        WrapDatabaseRef<&'db DB>: Database<Error = DatabaseError>,
    {
        let arc_env = EvmEnv::new(
            evm_env.cfg_env.clone().with_spec_and_mainnet_gas_params(self.arc_spec()),
            evm_env.block_env.clone(),
        );
        let mut evm = FoundryArcEvmFactory::new(self.arc_chain_spec(evm_env.cfg_env.chain_id))
            .create_evm_with_inspector(WrapDatabaseRef(db), arc_env, inspector);
        self.inject_precompiles(evm.precompiles_mut());
        Ok(evm.transact(tx_env)?)
    }
        ...
        WrapDatabaseRef<&'db DB>: Database<Error = DatabaseError>,
    {
        ...
        // Arc transactions are Ethereum-shaped, so they can't be distinguished by envelope type
        // like tempo/optimism above; dispatch on the node's Arc mode instead. Mirrors the
        // `self.is_arc()` branch in `transact_with_inspector_ref`.
        if self.is_arc() {
            let result = self.transact_arc_with_inspector_ref(db, evm_env, inspector, tx_env)?;
            return Ok((result, base));
        }
    ...
    fn arc_spec(&self) -> SpecId {
        self.hardfork.into()
    }

    pub(crate) fn arc_chain_spec(
        &self,
        chain_id: u64,
    ) -> std::sync::Arc<arc_execution_config::chainspec::ArcChainSpec> {
        foundry_evm::core::evm::arc_chainspec_for(&self.networks, chain_id)
    }

    /// Validates a user-supplied `--gas-limit` against the Arc protocol block gas limit at startup.
    ///
    /// Arc controls the block gas limit on-chain via ProtocolConfig and re-derives it every block,
    /// so `--gas-limit` is only meaningful as an assertion. This derives the value from the final
    /// startup state (genesis + fork + init state, all applied by the caller) and rejects a
    /// mismatching `--gas-limit`, restoring the "reject a non-protocol gas limit at startup"
    /// guarantee while, on a fork, comparing against the actual (possibly governance-adjusted)
    /// ProtocolConfig value instead of the chainspec default. `--disable-block-gas-limit` is
    /// rejected outright. No-op for non-Arc networks.
    pub(crate) async fn validate_arc_startup_gas_limit(
        &self,
        requested_gas_limit: Option<u64>,
        disable_block_gas_limit: bool,
    ) -> Result<(), BlockchainError> {
        if !self.is_arc() {
            return Ok(());
        }
        if disable_block_gas_limit {
            return Err(BlockchainError::Message(
                "--disable-block-gas-limit is not allowed on Arc; the block gas limit is controlled \
                 by ProtocolConfig"
                    .to_string(),
            ));
        }
        let Some(requested) = requested_gas_limit else {
            return Ok(());
        };

        let parent_env = self.evm_env.read().clone();
        let block_number = parent_env.block_env.number.saturating_to::<u64>().saturating_add(1);
        let chain_spec = self.arc_chain_spec(parent_env.cfg_env.chain_id);
        let derived = {
            let db = self.db.read().await;
            let mut overlay = AnvilCacheDB::new(&*db);
            foundry_evm::core::evm::arc_next_block_evm_env(
                &mut overlay,
                &chain_spec,
                &parent_env,
                foundry_evm::core::evm::ArcNextBlockAttributes {
                    number: block_number,
                    timestamp: self.time.current_call_timestamp(),
                },
            )
            .block_env
            .gas_limit
        ...
        if requested != derived {
            return Err(BlockchainError::Message(format!(
                "--gas-limit {requested} does not match the Arc protocol block gas limit \
                 ({derived}); Arc controls the block gas limit via ProtocolConfig"
            )));
        }
        Ok(())
    }

    async fn next_block_base_fee_from_parent_state<H: BlockHeader>(
        &self,
        parent: &H,
        block_gas_limit: u64,
    ) -> u64 {
        if !self.is_arc() {
            return self.fees.get_next_block_base_fee_per_gas(
                parent.gas_used(),
                block_gas_limit,
        ...
        let mut evm_env = self.evm_env.read().clone();
        evm_env.block_env = block_env_from_header(parent);
        evm_env.block_env.gas_limit = block_gas_limit;
        let chain_spec = self.arc_chain_spec(evm_env.cfg_env.chain_id);
    ...
    fn execute_with_block_executor<DB>(
        ...
        (
            ExecutedPoolTransactions<FoundryTxEnvelope>,
            BlockExecutionResult<FoundryReceiptEnvelope>,
            Option<u64>,
        ),
        BlockchainError,
    >
        ...
        DB: StateDB<Error = DatabaseError>,
    {
        ...
        macro_rules! run {
            ($evm:expr) => {{
                ...
                drop(evm);
                Ok((pool_result, block_result, None))
            }};
        }
        ...
        if self.is_tempo() {
            ...
            let chain_spec = self.arc_chain_spec(evm_env.cfg_env.chain_id);
            if run_arc_block_lifecycle && evm_env.block_env.beneficiary.is_zero() {
                return Err(BlockchainError::Message(
                    "Arc block pre-execution validation failed: zero beneficiary is not allowed"
                        .to_string(),
                ));
            }
            let mut evm = FoundryArcEvmFactory::new(chain_spec.clone())
                .create_evm_with_inspector(db, arc_env, inspector);
            if !run_arc_block_lifecycle {
                return run!(evm);
            }

            // Every other network reaches this through `run!`, which injects on its own. Arc's
            // block executor is built here instead, so the user-supplied precompiles and the
            // ecrecover cheatcode override have to be added before it takes the EVM.
            self.inject_precompiles(evm.precompiles_mut());
            ...
            let ctx = EthBlockExecutionCtx {
                ...
                tx_count_hint: Some(pool_transactions.len()),
                slot_number: None,
            };
            let receipt_builder = FoundryReceiptBuilder;
            let mut executor = ArcBlockExecutor::new(evm, ctx, &chain_spec, &receipt_builder);
            executor.apply_pre_execution_changes().map_err(|err| {
                BlockchainError::Message(format!(
                    "Arc block pre-execution validation failed: {err}"
                ))
            })?;
            let pool_result = execute_pool_transactions(
                &mut executor,
            ...
            let (mut evm, block_result) = executor.finish().map_err(|err| {
                BlockchainError::Message(format!("Arc block finalization failed: {err}"))
            })?;
            let block_number = evm_env.block_env.number.saturating_to();
            let next_base_fee = retrieve_gas_values(block_number, &mut evm)
                .map_err(|err| {
                    BlockchainError::Message(format!(
                        "Arc block finalization did not persist SystemAccounting gas values: {err}"
                    ))
                })?
                .nextBaseFee;
            Ok((pool_result, block_result, Some(next_base_fee)))
        } else {
            let mut evm =
    ...
    fn replay_block_transactions_with_inspector(
        ...
        if self.is_arc() {
            let chain_spec = self.arc_chain_spec(evm_env.cfg_env.chain_id);
            let mut evm = FoundryArcEvmFactory::new(chain_spec.clone())
                .create_evm(&mut cache_db, evm_env.clone());
            self.inject_precompiles(evm.precompiles_mut());
            let ctx = EthBlockExecutionCtx {
            ...
            if let Err(err) = executor.apply_pre_execution_changes() {
                warn!(
                    target: "backend",
                    %err,
                    "Arc pre-execution for trace replay failed; continuing replay"
                );
            }
        }
    ...
    async fn apply_genesis(&self) -> Result<(), DatabaseError> {
        ...
        if self.is_arc() && !self.is_fork() {
            let chain_spec = bundled_chainspec_for_chain_id(self.chain_id().to())
                .unwrap_or_else(|| LOCAL_DEV.clone());
            let arc_genesis = chain_spec.inner.genesis.clone();
            let db = self.db.write().await;
            self.genesis.apply_genesis_alloc(db, &arc_genesis)?;
        }

        // User-provided genesis allocations take precedence over Arc defaults.
        let db = self.db.write().await;
            ...
            // update all settings related to the forked block
            {
                if let Some(fork_url) = forking.json_rpc_url {
                    ...
                    let gas_limit = {
                        ...
                        node_config.configure_arc_fork_env(&mut env, fork.chain_id());
                        gas_limit
                    };

                    let next_block_base_fee = self
                        .next_block_base_fee_from_parent_state(&fork_block.header, gas_limit)
                        .await;
    ...
    pub async fn reset_to_in_mem(&self) -> Result<(), BlockchainError> {
        ...
        *self.blockchain.storage.write() = BlockchainStorage::new(
        ...
        // Reset fees to initial state. Arc restores its protocol base-fee floor (as at startup)
        // rather than the Ethereum initial fee, so the first block mined after reset does not start
        // below the floor and price out the transactions that follow it.
        if self.fees.is_eip1559() {
            let initial_base_fee = if self.is_arc() {
                let chain_id = self.evm_env.read().cfg_env.chain_id;
                foundry_evm::core::evm::arc_initial_base_fee(&self.arc_chain_spec(chain_id))
                    .unwrap_or(crate::eth::fees::INITIAL_BASE_FEE)
            } else {
                crate::eth::fees::INITIAL_BASE_FEE
            };
            self.fees.set_base_fee(initial_base_fee);
        }
    ...
    pub async fn inspect_tx(
        ...
        BlockchainError,
    > {
        // Acquire the db read guard first and hold it for the whole call. For Arc, derive the
        // candidate next-block env (gas limit included) from this exact snapshot on a throwaway
        // overlay, so the derive and the execution below linearize to one state instead of copying
        // a (possibly stale) env before locking the db.
        let db = self.db.read().await;
        let evm_env = if self.is_arc() {
            let parent_env = self.evm_env.read().clone();
            let block_number = parent_env.block_env.number.saturating_to::<u64>().saturating_add(1);
            let chain_spec = self.arc_chain_spec(parent_env.cfg_env.chain_id);
            let mut overlay = AnvilCacheDB::new(&*db);
            let mut env = foundry_evm::core::evm::arc_next_block_evm_env(
                &mut overlay,
                &chain_spec,
                &parent_env,
                foundry_evm::core::evm::ArcNextBlockAttributes {
            ...
            env.block_env.blob_excess_gas_and_price = self.excess_blob_gas_and_price();
            env
        } else {
            self.next_evm_env()
        };
    ...
    N: Network<TxEnvelope = FoundryTxEnvelope, ReceiptEnvelope = FoundryReceiptEnvelope>,
{
    ...
    fn build_block_info(
        ...
        let header = Header {
            ...
            extra_data: arc_next_base_fee.map(encode_base_fee_to_bytes).unwrap_or_default(),
    ...
    async fn do_mine_block(
        ...
        let (outcome, header, block_hash) = {
            ...
            let (block_info, included, invalid, not_yet_valid, block_hash) = {
                let mut db = self.db.write().await;
                let state_snapshot = self.is_arc().then(|| db.read_as_state_snapshot());
                let time_snapshot = self.is_arc().then(|| self.time.snapshot());

                // The next block timestamp is set just before execution so it is as close as
                // possible to actual execution time (concurrent requests can delay acquiring the db
                // lock).
                let next_timestamp = self.time.next_timestamp();
                if self.is_arc() {
                    // Arc governs the block gas limit on-chain via ProtocolConfig. Build the whole
                    // next-block env from the parent's committed state — number/timestamp/spec/
                    // gas_limit derived together — so the ProtocolConfig system call and this
                    // block's execution use the identical environment, and the block matches
                    // exactly
                    // what `ArcBlockExecutor::apply_pre_execution_changes` will require. This is
                    // derived under the same db write lock the block executes on, so it is a
                    // consistent snapshot; a governance change is followed on the very next block
                    // instead of the executor rejecting Anvil's own block and stalling production.
                    let chain_spec = self.arc_chain_spec(evm_env.cfg_env.chain_id);
                    evm_env = foundry_evm::core::evm::arc_next_block_evm_env(
                        &mut **db,
                        &chain_spec,
                        &evm_env,
                        foundry_evm::core::evm::ArcNextBlockAttributes {
                    ...
                    evm_env.block_env.timestamp = U256::from(next_timestamp);
                }
                ...
                let execution = self.execute_with_block_executor(
                    &mut **db,
                    &evm_env,
                    best_hash,
                    spec_id,
                ...
                let (pool_result, block_result, arc_next_base_fee) = match execution {
                    Ok(result) => result,
                    Err(err) => {
                        if let Some(state_snapshot) = state_snapshot {
                            db.clear();
                            db.init_from_state_snapshot(state_snapshot);
                        }
                        if let Some(time_snapshot) = time_snapshot {
                            self.time.restore(time_snapshot);
                        }
                        error!(
                            target: "backend",
                            block_number,
                            reason = %err,
                            "Arc block production aborted; state and time reverted"
                        );
                        return Err(err);
                    }
                };
        ...
        let next_block_base_fee =
            self.next_block_base_fee_from_parent_state(&header, header.gas_limit).await;
        ...
        F: FnOnce(Box<dyn MaybeFullDatabase + '_>, BlockInfo<N>) -> T,
    {
        ...
        // The pending block is a candidate child of latest. For Arc, derive its whole env
        // (number/timestamp/spec/gas_limit) from the latest committed state on the throwaway
        // overlay, under the db read guard held for the rest of this function — a consistent
        // snapshot matching what the block executor requires. Other networks keep the cached env.
        let evm_env = if self.is_arc() {
            let parent_env = self.evm_env.read().clone();
            let block_number = parent_env.block_env.number.saturating_to::<u64>().saturating_add(1);
            let chain_spec = self.arc_chain_spec(parent_env.cfg_env.chain_id);
        ...
        // The pending block executes against a throwaway `cache_db`, so a failed Arc block
        // lifecycle (e.g. blocklisted beneficiary or a block gas limit that does not match the
        // protocol value) needs no state rollback — surface it as an RPC error via `?` instead of
        // panicking the node.
        let (pool_result, block_result, arc_next_base_fee) = self.execute_with_block_executor(
            &mut cache_db,
            &evm_env,
            parent_hash,
            spec_id,
...
impl<N: Network<ReceiptEnvelope = FoundryReceiptEnvelope>> Backend<N> {
    ...
    pub async fn load_state(&self, state: SerializableState) -> Result<bool, BlockchainError> {
        ...
        if let Some(block) = state.block.clone() {
            ...
            if self.is_arc() {
                let node_config = self.node_config.read().await;
                let mut evm_env = self.evm_env.write();
                let chain_id = evm_env.cfg_env.chain_id;
                node_config.configure_arc_fork_env(&mut evm_env, chain_id);
            }
    ...
    N: Network<TxEnvelope = FoundryTxEnvelope, ReceiptEnvelope = FoundryReceiptEnvelope>,
{
    async fn validate_pool_transaction(
        ...
        if self.is_arc() && tx.transaction.as_ref().is_eip4844() {
            return Err(BlockchainError::Message(
                "Arc does not support blob transactions".to_string(),
            ));
        }

        if self.is_arc() {
            // Every Arc admission check reads the same latest-state snapshot under one db read
            // guard: NativeCoinControl blocklist, the sender account, and the
            // ProtocolConfig-derived next-block gas limit. Reading the account here
            // (rather than `get_account`, which re-locks) avoids a recursive read
            // acquisition that could deadlock against a waiting writer on the fair
            // RwLock, and keeps every check linearized to a single state.
            let db = self.db.read().await;

            let has_value = !tx.transaction.value().is_zero();
            let addresses =
                std::iter::once(*tx.sender()).chain(tx.transaction.to().filter(|_| has_value));
            for address in addresses {
                let slot = compute_is_blocklisted_storage_slot(address);
                let status = db
                    .storage_ref(NATIVE_COIN_CONTROL_ADDRESS, U256::from_be_bytes(slot.0))
                    .map_err(|e| {
                        BlockchainError::Message(format!(
                            "Arc NativeCoinControl storage read failed: {e}"
                        ))
                    })?;
                if is_blocklisted_status(status) {
                    return Err(BlockchainError::Message(format!(
                        "transaction rejected: address {address} is blocklisted by NativeCoinControl"
                    )));
                }
            }

            let account = db.basic_ref(*tx.sender())?.unwrap_or_default();

            // Derive the candidate next-block env (gas limit included) on a throwaway overlay of
            // the same guarded state. Admission is best-effort against current state: a
            // tx admitted here can still be excluded later by mining/pending, which
            // re-derive the limit and re-run this check, if a governance change lowers
            // the limit after admission.
            let parent_env = self.evm_env.read().clone();
            let block_number = parent_env.block_env.number.saturating_to::<u64>().saturating_add(1);
            let chain_spec = self.arc_chain_spec(parent_env.cfg_env.chain_id);
            let mut overlay = AnvilCacheDB::new(&*db);
            let mut evm_env = foundry_evm::core::evm::arc_next_block_evm_env(
                &mut overlay,
                &chain_spec,
                &parent_env,
                foundry_evm::core::evm::ArcNextBlockAttributes {
            ...
            // `arc_next_block_evm_env` carries the parent's base fee over for the caller to
            // finalize. Admission must price against the next block's fee (what mining will
            // enforce), not the parent's — otherwise a fee cap between the two is wrongly
            // rejected here. Mirror the mining/`transact` paths.
            evm_env.block_env.basefee = self.base_fee();
            evm_env.block_env.blob_excess_gas_and_price = self.excess_blob_gas_and_price();

            return Ok(self.validate_pool_transaction_for(tx, &account, &evm_env)?);
        }
...
mod tests {
    ...
    use arc_precompiles::system_accounting::{
        SYSTEM_ACCOUNTING_ADDRESS, compute_gas_values_storage_slot,
    };
    use foundry_evm_networks::NetworkConfigs;

    #[tokio::test]
    async fn arc_next_base_fee_falls_back_to_system_accounting_state() {
        let (api, _handle) =
            spawn(NodeConfig::test().with_networks(NetworkConfigs::with_arc())).await;
        let block_number = 10;
        let expected = 55_555u64;
        let packed = U256::from(1u64) | (U256::from(2u64) << 64) | (U256::from(expected) << 128);
        ...
        let next =
            api.backend.next_block_base_fee_from_parent_state(&header, header.gas_limit).await;

        assert_eq!(next, expected);
    }

    /// With no value in `SystemAccounting[n]`, the next base fee is recomputed from state.
    /// Upstream's `ArcBlockExecutor` is what fills that slot while mining, so the value it
    /// stored is the reference: mine through the Arc lifecycle, keep what it wrote, drop the
    /// slot, and require the recomputation to land on the same number.
    #[tokio::test]
    async fn arc_next_base_fee_recomputation_matches_the_block_executor() {
        // Started above the protocol floor on purpose: at the floor every answer clamps to it, and
        // the comparison would hold no matter what the formula did.
        let (api, handle) = spawn(
            NodeConfig::test()
                .with_networks(NetworkConfigs::with_arc())
                .with_base_fee(Some(80_000_000_000u64)),
        )
        .await;
        let provider = handle.http_provider();
        let from = handle.dev_wallets().next().unwrap().address();
        let to = handle.dev_wallets().nth(1).unwrap().address();

        // Two blocks, both carrying real gas: recomputing reads `SystemAccounting[n - 1]`, and on
        // empty blocks its `gasUsedSmoothed` is zero, which leaves the EMA input unexercised.
        for _ in 0..2 {
            provider
        ...
        let number = api.backend.best_number();
        let header = api.backend.get_block(number).expect("mined block").header;
        let expected = decode_base_fee_from_bytes(&header.extra_data)
            .expect("Arc mining carries the next base fee in extra_data");

        // Zeroing the slot leaves `nextBaseFee` at 0, which is what makes the lookup fall through.
        // `n` and `n - 1` land in different ring-buffer slots, so `n - 1` survives.
        let slot = compute_gas_values_storage_slot(number);
        api.anvil_set_storage_at(
            SYSTEM_ACCOUNTING_ADDRESS,
            U256::from_be_bytes(slot.0),
            B256::ZERO,
        ...
        // Strip the carrier too, or the caller answers from the header without recomputing.
        let mut parent = header;
        parent.extra_data = Default::default();

        let recomputed =
            api.backend.next_block_base_fee_from_parent_state(&parent, parent.gas_limit).await;

        assert_eq!(recomputed, expected, "recomputation diverged from the block executor");
    }

    /// A parent with no recorded gas values still yields a bounded fee.
    ///
    /// This is the shape a fork takes when the chain never wrote `SystemAccounting` for that height
    /// — a pre-Zero5 Arc block, or a chain that is not Arc at all. Reading an empty slot succeeds
    /// and returns zeros, so the recomputation runs with a smoothed gas of 0 rather than taking
    /// the `unwrap_or` guard, which only fires when the system call itself errors.
    #[tokio::test]
    async fn arc_next_base_fee_recomputes_without_recorded_gas_values() {
        let start = 80_000_000_000u64;
        let (api, handle) = spawn(
            NodeConfig::test().with_networks(NetworkConfigs::with_arc()).with_base_fee(Some(start)),
        )
        .await;
        let provider = handle.http_provider();
        let from = handle.dev_wallets().next().unwrap().address();
        ...
        let number = api.backend.best_number();
        let mut parent = api.backend.get_block(number).expect("mined block").header;
        parent.extra_data = Default::default();

        let clear = async |block: u64| {
            let slot = compute_gas_values_storage_slot(block);
            api.anvil_set_storage_at(
                SYSTEM_ACCOUNTING_ADDRESS,
                U256::from_be_bytes(slot.0),
                B256::ZERO,
        ...
        // Only `n` cleared: the parent's own smoothed gas is still on chain.
        clear(number).await;
        let with_parent_values =
            api.backend.next_block_base_fee_from_parent_state(&parent, parent.gas_limit).await;

        // `n - 1` cleared as well: nothing is left to smooth from.
        clear(number - 1).await;
        let without_parent_values =
            api.backend.next_block_base_fee_from_parent_state(&parent, parent.gas_limit).await;

        assert!(without_parent_values > 0, "a missing history must not produce a zero base fee");
        assert!(
            without_parent_values < start,
            "blocks this empty should price the next one below {start}, got {without_parent_values}"
        );
        assert_ne!(
            without_parent_values, with_parent_values,
            "the recorded smoothed gas has to reach the calculation, or this path proves nothing"
        );
    }
