...
mod test {
    ...
    #[test]
    fn cache_db_snapshot_restore_merges_snapshot_accounts() {
        let mut db = MemDb::default();
        let existing = Address::repeat_byte(1);
        let created = Address::repeat_byte(2);
        let created_contract = Address::repeat_byte(3);
        ...
        db.insert_account(created, AccountInfo { balance: U256::from(2), ..Default::default() });

        let code = Bytes::from(vec![0x60, 0x00]);
        let code_hash = keccak256(&code);
        db.insert_account(
            created_contract,
            AccountInfo { code_hash, code: Some(Bytecode::new_raw(code)), ..Default::default() },
        );
        assert!(db.inner.cache.accounts.contains_key(&created));
        assert!(db.inner.cache.accounts.contains_key(&created_contract));
        assert!(db.inner.cache.contracts.contains_key(&code_hash));

        db.init_from_state_snapshot(snapshot);

        assert!(db.inner.cache.accounts.contains_key(&existing));
        assert!(db.inner.cache.accounts.contains_key(&created));
        assert!(db.inner.cache.accounts.contains_key(&created_contract));
        assert!(db.inner.cache.contracts.contains_key(&code_hash));
    }
    ...
    #[test]
    fn cache_db_clear_then_snapshot_restore_removes_accounts_created_after_snapshot() {
        let mut db = MemDb::default();
        let existing = Address::repeat_byte(1);
        let created = Address::repeat_byte(2);
        let created_contract = Address::repeat_byte(3);
        ...
        // Arc aborts need replacement semantics, but the generic restore API remains a merge.
        db.clear();
        db.init_from_state_snapshot(snapshot);
