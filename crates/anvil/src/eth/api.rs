...
use arc_execution_config::{
    chainspec::BlockGasLimitProvider, protocol_config::PROTOCOL_CONFIG_ADDRESS,
};
use foundry_common::{
    provider::ProviderBuilder,
    version::{COMMIT_SHA, SEMVER_VERSION},
};
use foundry_evm::{core::evm::ARC_PROTOCOL_CONFIG_BLOCK_GAS_LIMIT_SLOT, decode::RevertDecoder};
...
impl<N: Network> EthApi<N> {
    ...
    pub async fn anvil_set_next_block_base_fee_per_gas(&self, basefee: U256) -> Result<()> {
        ...
        if self.backend.is_arc() {
            // On Arc the block base fee is derived from the parent block's carrier (encoded in its
            // `extra_data`); there is no settable source, so an override could only produce a block
            // the Arc consensus rules reject. Fail loudly instead of silently mining an invalid
            // one.
            return Err(RpcError::invalid_params(
                "the next block base fee cannot be overridden on Arc; it is derived from the parent \
                 block's carrier",
            )
            .into());
        }
    ...
    pub async fn anvil_node_info(&self) -> Result<NodeInfo> {
        ...
        Ok(NodeInfo {
            ...
            fork_config: fork_config
                ...
                .unwrap_or_default(),
            network: if self.backend.is_arc() {
                Some("arc".to_string())
            } else {
                self.backend.is_tempo().then(|| "tempo".to_string())
            },
        })
    }
    ...
    pub async fn evm_set_block_gas_limit(&self, gas_limit: U256) -> Result<bool> {
        ...
        let gas_limit = gas_limit.to();
        if self.backend.is_arc() {
            // On Arc the block gas limit is governed on-chain by ProtocolConfig, and
            // `ArcBlockExecutor::apply_pre_execution_changes` rejects any block whose limit does
            // not match `expected_gas_limit(ProtocolConfig, chainspec bounds)`. Merely
            // setting `block_env` (as eth/op/tempo do) would diverge from that
            // authority and stall production. To make this cheat actually change the
            // limit — as it does on the other networks — write the ProtocolConfig
            // storage slot (the source the executor reads) and sync `block_env` so the
            // very next block already matches. Resolve through the same helper as the
            // mining path so the user's selected Arc hardfork is honored, rather than
            // the raw bundled chainspec.
            let chain_spec = self.backend.arc_chain_spec(self.backend.chain_id().to());
            let next_number = self.backend.best_number().saturating_add(1);
            let config = chain_spec.block_gas_limit_config(next_number);
            // Reject out-of-bounds values: `expected_gas_limit` would clamp/fall back to the
            // default, so the value we wrote would silently diverge from what the executor
            // computes.
            if gas_limit < config.min() || gas_limit > config.max() {
                return Err(BlockchainError::Message(format!(
                    "Arc block gas limit must be within [{}, {}] (ProtocolConfig bounds); \
                     evm_setBlockGasLimit({gas_limit}) is out of range",
                    config.min(),
                    config.max()
                )));
            }
            self.backend
                .set_storage_at(
                    ...
                    U256::from_be_bytes(ARC_PROTOCOL_CONFIG_BLOCK_GAS_LIMIT_SLOT.0),
                    B256::from(U256::from(gas_limit).to_be_bytes::<32>()),
                )
                .await?;
        }
    ...
    pub async fn execute(&self, request: EthRequest) -> ResponseResult {
        ...
        let response = match request.clone() {
            ...
            EthRequest::EvmSetBlockGasLimit(gas_limit) => {
                self.evm_set_block_gas_limit(gas_limit).await.to_rpc_result()
            }
...
impl EthApi<FoundryNetwork> {
    ...
    pub async fn mine_one(&self) {
        if let Err(err) = self.try_mine_one().await {
            warn!(target: "node", %err, "failed to mine block");
        }
    }

    /// Mines exactly one block and reports consensus-level validation failures.
    pub async fn try_mine_one(&self) -> Result<()> {
        if self.backend.is_arc() && self.backend.coinbase().is_zero() {
            return Err(BlockchainError::Message(
                "Arc block production aborted: block beneficiary must not be the zero address"
                    .to_string(),
            ));
        }
