...
type MiningResult<N> = (
    Result<MinedBlockOutcome<<N as Network>::TxEnvelope>, crate::eth::error::BlockchainError>,
    Arc<Backend<N>>,
);
        ...
        if let Some(mut mining) = pin.block_mining.take() {
            if let Poll::Ready(res) = mining.poll_unpin(cx) {
                return match res {
                    ...
                    Ok((Err(err), backend)) => {
                        warn!(target: "miner", %err, "block production aborted");
                        pin.idle_backend = Some(backend);
                        // This does not retry the failed set: it was removed from `queued` when the
                        // build was spawned, and the miner won't re-emit it until a new ready-tx
                        // notification clears its `has_pending_txs` latch. The wake is needed
                        // because polling the `JoinHandle` above is the only place this stream
                        // hands out its waker, and that handle is now consumed — returning
                        // `Pending` without waking would leave any *other* sets still in `queued`
                        // (they accumulate while a build is in flight) with nothing to poll them.
