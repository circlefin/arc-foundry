...
pub async fn try_spawn(mut config: NodeConfig) -> Result<(EthApi<FoundryNetwork>, NodeHandle)> {
    ...
    // Capture the user's requested block gas limit before `setup` runs: fork setup overwrites
    // `config.gas_limit` with the forked chain's value, so this is the only point it is still the
    // original `--gas-limit`.
    let requested_gas_limit = config.gas_limit;
    let requested_disable_block_gas_limit = config.disable_block_gas_limit;
    ...
    // Validate the requested gas limit once, after every state source (genesis, fork, init state)
    // is in place, against the ProtocolConfig value derived from that final state. Doing it
    // here rather than during setup avoids rejecting a value that a later `init_state` load
    // would have made legal. No-op for non-Arc networks.
    backend
        .validate_arc_startup_gas_limit(requested_gas_limit, requested_disable_block_gas_limit)
        .await?;
