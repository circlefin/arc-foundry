...
use foundry_config::{
    Chain, Config, FromEvmVersion, NamedChain, error::ExtractConfigError, figment::Figment,
};
use foundry_evm::{
    core::evm::{FoundryEvmNetwork, arc_unsupported_evm_spec},
...
pub trait LoadConfig {
    ...
    fn load_config_no_warnings(&self) -> Result<Config, ExtractConfigError> {
        ...
        reject_unsupported_arc_evm_version(&config)?;
        Ok(config)
    }
    ...
    fn load_config_and_evm_opts_no_warnings(&self) -> Result<(Config, EvmOpts)> {
        ...
        evm_opts.apply_arc_hardfork(config.hardfork);
...
/// Returns the spec Arc will execute when the configured `evm_version` compiles above it.
///
/// Uses the normalized version, which is only ever lowered to fit the selected solc, so
/// normalization cannot turn an acceptable value into a rejected one.
fn unsupported_arc_evm_spec(config: &Config) -> Option<SpecId> {
    arc_unsupported_evm_spec(
        &config.networks,
        config.chain.map_or(foundry_common::DEV_CHAIN_ID, |chain| chain.id()),
        SpecId::from_evm_version(config.get_normalized_evm_version()),
    )
}

fn unsupported_arc_evm_spec_error(config: &Config, arc_spec: SpecId) -> ExtractConfigError {
    ExtractConfigError::new(foundry_config::figment::Error::from(format!(
        "evm_version `{}` is newer than the EVM Arc executes (`{}`); solc would emit instructions \
         this chain cannot run. Lower evm_version, or use hardfork to select a different Arc \
         release.",
        config.get_normalized_evm_version(),
        format!("{arc_spec:?}").to_lowercase(),
    )))
}

/// Rejects an `evm_version` that solc would compile above the EVM Arc executes.
///
/// Applies regardless of whether the value was configured or defaulted: the artifact is equally
/// unrunnable either way, and `Config`'s default tracks the newest release, so it can outrun Arc's
/// schedule on its own.
fn reject_unsupported_arc_evm_version(config: &Config) -> Result<(), ExtractConfigError> {
    if let Some(arc_spec) = unsupported_arc_evm_spec(config) {
        return Err(unsupported_arc_evm_spec_error(config, arc_spec));
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn arc_conflict_is_reported_only_for_arc_with_a_newer_evm_version() {
        // Zero4 is the only schedule that leaves Osaka unactivated, which is how an Arc chain
        // executing Prague gets constructed here. It is not locally selectable and does not need to
        // be: this exercises the chainspec-derived spec, not what `--hardfork` accepts.
        let arc_on_prague = Config {
            networks: NetworkConfigs::with_arc()
                .with_arc_hardfork(foundry_evm::hardfork::ArcHardfork::Zero4),
            evm_version: foundry_compilers::artifacts::EvmVersion::Osaka,
            ..Config::default()
        };
        assert_eq!(unsupported_arc_evm_spec(&arc_on_prague), Some(SpecId::PRAGUE));

        let message = unsupported_arc_evm_spec_error(&arc_on_prague, SpecId::PRAGUE).to_string();
        assert!(message.contains("osaka"), "names the configured version: {message}");
        assert!(message.contains("prague"), "names what Arc executes: {message}");

        // Same `evm_version`, but the local default activates Osaka.
        let arc_on_osaka = Config { networks: NetworkConfigs::with_arc(), ..arc_on_prague.clone() };
        assert_eq!(unsupported_arc_evm_spec(&arc_on_osaka), None);

        // And nothing is reported off Arc.
        let ethereum = Config { networks: NetworkConfigs::default(), ..arc_on_prague };
        assert_eq!(unsupported_arc_evm_spec(&ethereum), None);
    }
